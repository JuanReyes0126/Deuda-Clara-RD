...
export function Providers({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if ("serviceWorker" in navigator) {
      ...
      // Escuchar actualizaciones del Service Worker (con tipado seguro)
      navigator.serviceWorker.addEventListener("updatefound", () => {
        const newWorker = (navigator.serviceWorker as any).active || (navigator.serviceWorker as any).installing;
        if (newWorker) {
